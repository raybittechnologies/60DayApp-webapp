...
//       {/* SVG notched background — use xMidYMid meet to preserve shape on all screen widths */}
//       <svg
//         className="absolute inset-0 w-full h-full"
//         viewBox={`0 0 ${W} ${H}`}
//         fill="none"
//         xmlns="http://www.w3.org/2000/svg"
//         preserveAspectRatio="xMidYMid meet"  /* FIX: was "none" — caused shape distortion on narrow/wide screens */
//       >
...
//   return (
//     /* FIX: removed overflow-hidden — it was clipping phone mockup images that extend beyond section bounds */
...
//         {/* ── DESKTOP LAYOUT ── */}
//         {/* FIX: added xl:gap-[36px] and made left card flex-none with relative sizing via min-w/max-w 
//             instead of a hard 465px so it doesn't overflow at 1024–1100px viewport widths */}
//         <div className="hidden lg:flex relative w-full max-w-[1193px] mx-auto items-stretch gap-5 xl:gap-[36px]">

//           {/* Left card — FIX: replaced fixed w-[465px] with responsive clamp so it never overflows */}
//           <div
...
//               {/* FIX: removed fixed w-[442px] — use full available width instead */}
...
//             {/* FIX: replaced fixed pr-[140px] with responsive pr — was squishing text on 1024–1200px screens */}
...
//               </div>
//               {/* FIX: pr-[140px] → pr-[145px] lg:pr-[110px] xl:pr-[140px] to prevent text going under the number */}
...
//               </div>
//               {/* FIX: same pr fix as card 02 above */}
//               <div className="relative z-0 pt-[24px] pb-[32px] pl-[32px] pr-[145px] lg:pr-[110px] xl:pr-[145px] flex flex-col h-full justify-between">
//                 <div className="flex flex-col gap-[16px]">
//                   <div className="flex items-center gap-[16px]">
...
const MobileCard = ({ number, icon, title, desc, statValue, statLabel }) => {
  ...
  // Notch dimensions (fixed px, same feel as desktop)
  const W = 400;
  const H = height;
  const notchW = 96;   // width of the notch cut-out
  const notchH = 64;   // height of the notch cut-out
  const r = 16;        // inner notch corner radius
  const R = 20;        // outer card corner radius
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      {/* SVG notched background — stretches to match content height */}
      <svg
        className="absolute inset-0 w-full h-full"
        viewBox={`0 0 ${W} ${H}`}
        fill="none"
      ...
      {/* Number in notch — top-right, sized to match notchW × notchH */}
      <div
        className="absolute top-0 right-0 z-10 flex items-center justify-center"
        style={{ width: `${(notchW / W) * 100}%`, height: `${notchH}px` }}
      >
        <GlassNumber id={`m${number}`} value={number} width={84} height={60} fontSize={52} />
      </div>

      {/* Card content */}
      <div className="relative z-0 flex flex-col gap-[10px] px-5 pb-5"
        style={{ paddingTop: `${notchH + 10}px` }}>
        {/* Icon + title + desc */}
        <div className="flex flex-col gap-[8px]">
          <div className="w-[38px] h-[38px] rounded-full bg-[#F05A28] flex items-center justify-center shadow-[0_0_16px_rgba(240,90,40,0.4)] shrink-0">
            {icon}
          </div>
          <h3 className="font-heading font-bold text-[20px] sm:text-[22px] tracking-tight text-[#1A1A1A] leading-tight">
            {title}
          </h3>
          <p className="font-body text-[13px] sm:text-[14px] leading-[1.6] text-[#1A1A1A]">
            {desc}
          </p>
        </div>
