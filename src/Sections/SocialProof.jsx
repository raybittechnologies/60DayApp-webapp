...
const SummaryCard = ({ title, subtitle, icon }) => (
  <div className="flex-1 flex flex-col items-center justify-center min-h-[280px] bg-[#FDF0EB] rounded-[28px] border-[1.5px] border-[#FF80551A] shadow-[0_4px_24px_rgba(230,120,80,0.08)] px-[32px] py-[48px]">
    <div className="w-[64px] h-[64px] rounded-full bg-[#FA764E] flex items-center justify-center text-white shadow-md mb-[32px]">
      {icon}
    </div>
    <h3 className="font-heading font-extrabold text-[36px] text-[#000000] leading-none mb-[14px] tracking-[-1px]">{title}</h3>
    <p className="font-body text-[18px] text-[#1A1A1A] font-400 tracking-wide">{subtitle}</p>
  </div>
);

const Badge = ({ icon, text, className }) => (
  <div className={`flex items-center gap-[8px] px-[20px] py-[10px] bg-gradient-to-r from-[#FFF5EF] to-[#FEF0E8] border-[1.5px] border-[#FAD0BF] rounded-full shadow-sm text-[#F05A28] ${className}`}>
...
const TestimonialCard = ({ name, role, text, imgUrl }) => {
  return (
<div className="relative w-full max-w-[501px] mx-auto" style={{ marginBottom: '40px', marginTop: '20px' }}>
      {/* Opening quote — top left corner */}
      <span className="absolute top-10 left-10 -translate-x-1/2 -translate-y-1/2 font-serif font-black text-[200px] text-[#FF8055] leading-none select-none z-30">
        &ldquo;
      </span>

      {/* Avatar — top right */}
      <div className="absolute -top-6 right-8 w-[110px] h-[110px] rounded-full border-[6px] border-[#FF8055] overflow-hidden z-40 shadow-xl bg-white">
      ...
      <div className="relative">
        ...
        {/* Content */}
        <div className="relative z-20 px-10 pt-10 pb-8">
          <div className="mb-4">
            <h4 className="text-[26px] font-extrabold text-black leading-tight">{name}</h4>
            <p className="text-[#808080] text-base font-medium mt-0.5">{role}</p>
          </div>
          <p className="text-[#1A1A1A] text-[17px] leading-[1.65] mb-6 font-normal">
            {text}
          </p>
          <div className="flex gap-1">
            {[...Array(5)].map((_, i) => (
              <svg key={i} width="24" height="24" viewBox="0 0 24 24" fill="#FA9E0D">
      ...
      <span className="absolute bottom-[-30px] right-1 translate-x-1/2 translate-y-1/2 font-serif font-black text-[200px] text-[#F3764E] leading-none select-none z-30">
        &rdquo;
      </span>
    </div>
  );
...
export default function SocialProof() {
  ...
  return (
    <section className="relative w-full flex justify-center items-start pb-[24px] overflow-hidden">
      <div className="w-full max-w-[1280px] bg-white pb-[24px] pt-[36px] flex flex-col items-center rounded-[24px]">
        ...
        <div className="flex flex-col items-center max-w-[800px] mb-[64px]">
          ...
          <p className="font-heading font-extrabold text-[20px] text-[#1A1A1A] text-center mt-[24px]">
        ...
        <div className="w-full max-w-[1080px] flex items-center gap-[16px] md:gap-[24px]">
...
<div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-[32px] px-[10px] overflow-visible">            <TestimonialCard
              name="Carolyn Willms"
