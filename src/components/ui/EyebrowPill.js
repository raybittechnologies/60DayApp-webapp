...
export default function EyebrowPill({ variant = 'nav', children }) {
  ...
  const NavItem = ({ to, children }) => {
    ...
    const isActive = 
      pathname === to || 
      (to !== '/' && to !== '/#how' && pathname.startsWith(to)) ||
      (to === '/#how' && pathname === '/');
    return (
      <Link 
        href={to} 
        className={`relative font-body text-[18px] leading-[1.7] whitespace-nowrap transition-all duration-300 hover:text-brand-orange flex flex-col items-center ${
          isActive ? 'text-brand-orange font-bold' : 'text-text-primary font-normal'
        }`}
      >
        {children}
        {isActive && (
  ...
  return (
    <div className="
      flex items-center justify-center gap-4 lg:gap-6 xl:gap-14
      px-4 lg:px-6 xl:px-14
      w-full max-w-[727px]
      h-[71px]
      rounded-full
      bg-brand-orange/10
